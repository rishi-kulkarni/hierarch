...
class DataSimulator:
    ...
    def generate(self):
        ...
        output[:, -1] = gen_fake_data(self.container, self.parameters,
...
def gen_fake_data(reference, params, return_all=False, seed=None):
    ...
    for i in range(reference.shape[1] - 1):
        ...
        if type(params[i][0]) is not int:
            ...
            idx, replicates = np.unique(reference[:, :i+1],
                                        return_counts=True, axis=0)
            ranlist = []
            if not isinstance(params[i][0], list):
                for j in range(len(idx)):
                    ranlist.append(params[i][0].rvs(*params[i][1:],
                                   random_state=rng))
                ranlist = np.repeat(ranlist, replicates)
            else:
                for j in range(len(idx)):
                    ranlist.append(params[i][j][0].rvs(*params[i][j][1:],
                                                       random_state=rng))
                ranlist = np.repeat(ranlist, replicates)
...
def make_ref_container(samples_per_level=[]):
    ...
    for i in range(1, len(iterator)):
        ...
        hcontainer = np.append(hcontainer,
                               np.array(append).reshape(len(append), 1),
                               axis=1)
    hcontainer = np.append(hcontainer,
                           np.zeros_like(hcontainer[:, 0]).reshape(
                            hcontainer[:, 0].shape[0], 1), axis=1)
