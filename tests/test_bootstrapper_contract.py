"""Contract of the nested bootstrap (Bootstrapper), independent of implementation.
...
class TestBootstrapperContract:
    ...
    def test_weights_only_touch_y(self, design, kind):
        _, hier, data = design
        boot = Bootstrapper(random_state=0, kind=kind)
        boot.fit(data)
        for start in range(1, data.shape[1] - 1):
            out = boot.transform(data, start=start)
    ...
    def test_efron_weights_are_nonnegative_integers(self, design):
        ...
        for start in range(1, data.shape[1] - 1):
            for _ in range(20):
                w = boot.transform(data, start=start)[:, -1]
                assert np.all(w >= 0)
    ...
    def test_bayesian_weights_are_nonnegative_and_continuous(self, design):
        ...
        data = data.copy()
        data[:, -1] = 1.0
        boot = Bootstrapper(random_state=1, kind="bayesian")
        boot.fit(data)
        w = boot.transform(data, start=1)[:, -1]
        assert np.all(w >= 0)
    ...
    def test_weight_mass_is_conserved_within_each_unresampled_cluster(
        ...
        data = data.copy()
        data[:, -1] = 1.0
        boot = Bootstrapper(random_state=3, kind=kind)
        boot.fit(data)
        for start in range(1, data.shape[1] - 1):
    ...
    def test_weight_mass_conserved_in_expectation_when_unbalanced(self, kind):
        ...
        for _ in range(reps):
            acc += np.bincount(ids, weights=boot.transform(data, start=1)[:, -1])
    ...
    def test_each_row_has_unit_expected_weight(self, design):
        ...
        data = data.copy()
        data[:, -1] = 1.0
        boot = Bootstrapper(random_state=5, kind="weights")
        boot.fit(data)
        reps = 3000
        acc = np.zeros(len(data))
        for _ in range(reps):
            acc += boot.transform(data, start=1)[:, -1]
        mean_w = acc / reps
    ...
    def test_last_level_efron_weights_are_multinomial(self):
        ...
        data[:, -1] = 1.0
        boot = Bootstrapper(random_state=8, kind="weights")
        boot.fit(data)
        start = data.shape[1] - 2  # last design column
        reps = 4000
        draws = np.array(
            [boot.transform(data, start=start)[:, -1] for _ in range(reps)]
        )
    ...
    def test_bayesian_last_level_weights_are_dirichlet(self):
        ...
        data = make_design([2, 3, k], rng=11)
        data[:, -1] = 1.0
        boot = Bootstrapper(random_state=8, kind="bayesian")
        boot.fit(data)
    ...
    def test_nesting_zero_parent_zeroes_all_descendants(self):
        ...
        data[:, -1] = 1.0
        boot = Bootstrapper(random_state=4, kind="weights")
        boot.fit(data)
    ...
    def test_skipping_a_level_gives_children_their_parents_weight(self):
        ...
        data[:, -1] = 1.0
        boot = Bootstrapper(random_state=6, kind="weights")
        boot.fit(data, skip=[2])  # last level not resampled
        ...
        assert any(np.any(boot.transform(data, start=1)[:, -1] != 1) for _ in range(20))

    def test_skipping_every_level_is_the_identity(self):
        data = make_design([2, 3, 4], rng=9)
        boot = Bootstrapper(random_state=6, kind="weights")
        boot.fit(data, skip=[1, 2])
        for _ in range(5):
            np.testing.assert_array_equal(boot.transform(data, start=1), data)
    ...
    def test_seed_reproducibility_and_variation(self, design):
        ...
        def draws(seed):
            b = Bootstrapper(random_state=seed, kind="weights")
            b.fit(data)
            return [b.transform(data, start=1) for _ in range(3)]
    ...
    def test_successive_transforms_differ(self, design):
        _, hier, data = design
        boot = Bootstrapper(random_state=0, kind="weights")
        boot.fit(data)
        outs = [boot.transform(data, start=1)[:, -1].tobytes() for _ in range(10)]
